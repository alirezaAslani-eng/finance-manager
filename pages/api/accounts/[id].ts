...
const handler: handler_type = async (req, res) => {
  ...
  switch (req.method as "DELETE" | "PUT" | "GET") {
    case "DELETE": {
      // TODO -> auth : who is deleting this document
    ...
    case "PUT": {
      ...
      const update_res = await editAccount(req.query.id, {
        ...
        cardNumber,
        currentBalance,
        user: "68a709aa701fc361de471a30", // TODO -> auth : who is updtaing this document
      });
      // * Check if dose the (id) exist ============= >
      clientError(
        `there is no document with this id : ${req.query.id}`, // ! Might throw Error <<<<<<<<
    ...
    case "GET": {
      // TODO -> auth : who wants to see a details of an account (is it user at all)
